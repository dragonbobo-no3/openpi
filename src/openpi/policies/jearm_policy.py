...
class JeArmInputs(transforms.DataTransformFn):
    """Inputs for the JeArm policy.

    Expected inputs when use_images=True:
    - images: dict[name, img] where img is [C, H, W]. name must be in EXPECTED_CAMERAS.
    - state: [16]
    - actions: [action_horizon, 16]  (optional during inference)
    """
...
@dataclasses.dataclass(frozen=True)
class JeArmOutputs(transforms.DataTransformFn):
    """Outputs for the JeArm policy."""
